...
uint32_t generate_monster_descriptions(dungeon_t *d){
    // note d.max_monsters must be >= 10 or we'll seg fault here
    ...
    m.set(name, desc, symb, color, speed, abil, hp, dam);
    for(i = 0; i < d->max_monsters - 5; i++){
       d->monster_descriptions.push_back(m);
    }

    name = "Amazon Lich Queen";
    desc = "description of queen";
    symb = 'm';
    abil = NPC_SMART & NPC_TELEPATH;
    color.push_back(0);
    speed.set(7, 1, 4);
    dam.set(0, 1, 4);
    hp.set(2, 2, 6);
    ...
    m.set(name, desc, symb, color, speed, abil, hp, dam);
    for(i = d->max_monsters - 5; i < d->max_monsters; i++){
       d->monster_descriptions.push_back(m);
    }
}

uint32_t generate_object_descriptions(dungeon_t *d){
    ...
    o.set(name, description, type, color, hit, damage, dodge, defence, weight, speed, attribute, value);
